...
With[{Z=Length[Tp]-1},
Module[
{zeros=DiscreteZero2[Transpose@{Range[0,1,1/Z],Tp-Tm}], CoI={1,Z+1},Pp=1.,Pm=1.},
If[Length[zeros]>0,
...
,Print["Error: Unequal lenghts."]];


StatDESML[s_,Z_,T_,par_List]:=
Module[{CoIp=Table[{{0}},{i,1,Binomial[s,2]}],normp=Table[{{0}},{i,1,Binomial[s,2]}],\[Rho]=Table[{{0}},{i,1,Binomial[s,2]}],CoINorm={{{0,0,0},{0.,0.}}},pair=0,CoIIndex={0},zeros={{0,0.,1.,1.}},CoI={{0}},TMat, vec={0.}},
...
Do[If[CoINorm[[i,2,1]]<0,vec*=Z Sqrt[2\[Pi] (CoINorm[[i,2,2]]/-CoINorm[[i,2,1]]) ]];
,{i,1,Length@vec}];
